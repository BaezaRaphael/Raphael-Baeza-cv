      ...
      <div className="grid md:grid-cols-3 gap-6">
        {dict.categories.map((cat, i) => (
          <Reveal key={cat.name} delay={i * 0.1}>
            <div className="p-8 rounded-2xl border border-white/10 bg-white/[0.02] hover:bg-white/[0.04] hover:border-white/20 transition-all duration-300 h-full">
              <h3 className="text-xs font-mono uppercase tracking-[0.2em] text-zinc-500 mb-6">
                {cat.name}
              </h3>
              <ul className="space-y-3">
                {cat.items.map((item) => (
                  <li key={item} className="text-base text-zinc-200 flex items-center gap-2">
